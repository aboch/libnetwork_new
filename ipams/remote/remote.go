...
func (a *allocator) RequestPool(addressSpace, pool, subPool string, options map[string]string, v6 bool) (string, *net.IPNet, map[string]string, error) {
	req := &api.RequestPool{AddressSpace: addressSpace, Pool: pool, SubPool: subPool, Options: options, V6: v6}
	res := &api.RequestPoolRes{}
	if err := a.call("RegisterPool", req, res); err != nil {
		return "", nil, nil, err
	}
...
func (a *allocator) ReleasePool(poolID string) error {
	req := &api.ReleasePool{PoolID: poolID}
	res := &api.ReleasePoolRes{}
...
func (a *allocator) RequestAddress(poolID string, address net.IP, options map[string]string) (*net.IPNet, map[string]string, error) {
	req := &api.RequestAddress{PoolID: poolID, Address: address, Options: options}
	res := &api.RequestAddressRes{}
...
func (a *allocator) ReleaseAddress(poolID string, address net.IP) error {
	req := &api.ReleaseAddress{PoolID: poolID, Address: address}
	res := &api.ReleaseAddressRes{}
