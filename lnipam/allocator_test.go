...
func TestAddSubnets(t *testing.T) {
	...
	err = a.AddSubnet("abc", &ipam.SubnetInfo{Subnet: sub0})
	if err != nil {
	...
	err = a.AddSubnet("default", &ipam.SubnetInfo{Subnet: sub1})
	if err == nil {
		t.Fatalf("Failed to detect overlapping subnets: %s and %s", sub0, sub1)
	}

	_, sub2, _ := net.ParseCIDR("10.128.0.0/9")
	err = a.AddSubnet("default", &ipam.SubnetInfo{Subnet: sub2})
	if err == nil {
	...
	err = a.AddSubnet("default", &ipam.SubnetInfo{Subnet: sub6})
	if err != nil {
...
func TestRemoveSubnet(t *testing.T) {
	...
	for _, i := range input {
		...
		err = a.AddSubnet(i.addrSpace, &ipam.SubnetInfo{Subnet: sub})
		if err != nil {
...
func TestRequestSyntaxCheck(t *testing.T) {
	...
	a.AddSubnet(addSpace, &ipam.SubnetInfo{Subnet: sub})
	...
	req = &ipam.AddressRequest{Address: net.ParseIP("172.17.0.23")}
	_, err = a.Request(addSpace, req)
	if err == nil {
...
func TestRelease(t *testing.T) {
	...
	for err != ipam.ErrNoAvailableIPs {
...
func benchmarkRequest(subnet *net.IPNet) {
	...
	req := &ipam.AddressRequest{Subnet: *subnet}
	for err != ipam.ErrNoAvailableIPs {
		_, err = a.Request("default", req)
