// Package lnipam provides a ipv4/ipv6 IPAM Service for clients.
package lnipam
...
type subnetKey struct {
	addressSpace ipam.AddressSpace
	subnet       string
}
...
type subnetData struct {
	info        *ipam.SubnetInfo // Configured subnet
...
func (a *Allocator) AddSubnet(addrSpace ipam.AddressSpace, subnetInfo *ipam.SubnetInfo) error {
	// Sanity check
	if addrSpace == "" {
		return ipam.ErrInvalidAddressSpace
	}
	if subnetInfo == nil || subnetInfo.Subnet == nil {
		return ipam.ErrInvalidSubnet
	}
	if a.contains(addrSpace, subnetInfo) {
		return ipam.ErrOverlapSubnet
	}
...
func adjustAndCheckSubnetSize(subnet *net.IPNet) (*net.IPNet, error) {
	...
	if v6 == getAddressVersion(subnet.IP) {
		if ones < minNetSizeV6 {
			return nil, ipam.ErrInvalidSubnet
		}
...
func (a *Allocator) contains(space ipam.AddressSpace, subInfo *ipam.SubnetInfo) bool {
	a.Lock()
	defer a.Unlock()
...
func (a *Allocator) RemoveSubnet(addrSpace ipam.AddressSpace, subnet *net.IPNet) error {
	if addrSpace == "" {
		return ipam.ErrInvalidAddressSpace
	}
	if subnet == nil {
...
func (a *Allocator) Request(addrSpace ipam.AddressSpace, req *ipam.AddressRequest) (*ipam.AddressResponse, error) {
	return a.request(addrSpace, req, v4)
}

// RequestV6 requesting an IPv6 address from the specified address space
func (a *Allocator) RequestV6(addrSpace ipam.AddressSpace, req *ipam.AddressRequest) (*ipam.AddressResponse, error) {
	return a.request(addrSpace, req, v6)
}

func (a *Allocator) request(addrSpace ipam.AddressSpace, req *ipam.AddressRequest, version ipVersion) (*ipam.AddressResponse, error) {
	// Empty response
	response := &ipam.AddressResponse{}

	// Sanity check
	if addrSpace == "" {
		return response, ipam.ErrInvalidAddressSpace
	}
	...
	if &req.Subnet != nil && version != getAddressVersion(req.Subnet.IP) {
		return response, ipam.ErrInvalidRequest
	}
...
func (a *Allocator) Release(addrSpace ipam.AddressSpace, address net.IP) {
	if address == nil {
		return
	}
...
func (a *Allocator) reserveAddress(addrSpace ipam.AddressSpace, subnet *net.IPNet, prefAddress net.IP, ver ipVersion) (net.IP, *net.IPNet, error) {
	var keyList []*isKey
	...
	if len(keyList) == 0 {
		return nil, nil, ipam.ErrNoAvailableSubnet
	}
	...
	return nil, nil, ipam.ErrNoAvailableIPs
}

// Get the list of available internal subnets for the specified address space and the desired ip version
func (a *Allocator) getSubnetList(addrSpace ipam.AddressSpace, ver ipVersion) []*isKey {
	var list [1024]*isKey
	ind := 0
	a.Lock()
...
func printLine(head *rleseq.Sequence) {
	fmt.Println()
	for head != nil {
		fmt.Printf("-")
		head = head.Next
